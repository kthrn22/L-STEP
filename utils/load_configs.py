...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--model_name', type=str, default='LSTEP', help='name of the model, note that EdgeBank is only applicable for evaluation')
    ...
    parser.add_argument('--transformer_depth', type=int, default=12, help='depth of linear transformer')
    parser.add_argument('--concat_pe', action='store_true', default=False, help='concat positioanl encoding before transformer')
    parser.add_argument('--pe_weight', type=float, default=0.5, help = 'weight for positional encoding loss')
    parser.add_argument('--neg_sample_weight', type=float, default=0.3, help='weight of negative links in positional encoding loss')
    ...
    parser.add_argument('--pre_trained', type = str, default = 'False', help='load pre-trained and keep training')
    parser.add_argument('--ablation', type = str, default = 'node_edge_feat', choices = ['fourier', 'pe_init', 'node_edge_feat', 'time_gap', "num_neighbors", 'no_pe', 'pe_weight', 'neg_sample_weight', 'num_fft_batches', 'weighted_sum'])
    parser.add_argument('--start_seed', type = int, default = 0)
    parser.add_argument('--end_seed', type = int, default = 4)

    ### TGB stuffs
    # parser.add_argument('--tgb_name', type=str, help='dataset to be used', default='tgbl-review',
    #                     choices=['wikipedia', 'reddit', 'mooc', 'lastfm', 'myket', 'enron', 'SocialEvo', 'uci', 'Flights', 'CanParl', 'USLegis', 'UNtrade', 'UNvote', 'Contacts'])
    try:
...
def load_link_prediction_best_configs_LSTEP(args: argparse.Namespace):
    best_args_dict = {
        'wikipedia':    {"num_fft_batches": 100, "time_gap": 1000, "num_neighbors": 15, "batch_size": 128},
        'reddit':       {"num_fft_batches": 100, "time_gap": 1000, "num_neighbors": 20, "batch_size": 200}, 
        'mooc':         {"num_fft_batches": 100, "time_gap": 2000, "num_neighbors": 30, "batch_size": 128}, 
        'lastfm':       {"num_fft_batches": 100, "time_gap": 1000, "num_neighbors": 30, "batch_size": 128}, 
        'enron':        {"num_fft_batches": 100, "time_gap": 1000, "num_neighbors": 20, "batch_size":  64}, 
        'SocialEvo':    {"num_fft_batches": 100, "time_gap": 1000, "num_neighbors": 20, "batch_size": 128}, 
        'uci':          {"num_fft_batches": 200, "time_gap":  500, "num_neighbors": 30, "batch_size": 100}, 
        'Flights':      {"num_fft_batches": 100, "time_gap": 1000, "num_neighbors": 30, "batch_size": 128}, 
        'CanParl':      {"num_fft_batches":  20, "time_gap":    2, "num_neighbors": 10, "batch_size": 64}, 
        'USLegis':      {"num_fft_batches":  50, "time_gap":    2, "num_neighbors": 10, "batch_size": 200}, 
        'UNtrade':      {"num_fft_batches": 200, "time_gap":    6, "num_neighbors": 30, "batch_size": 200}, 
        'UNvote':       {"num_fft_batches": 100, "time_gap":   10, "num_neighbors": 20, "batch_size": 128}, 
        'Contacts':     {"num_fft_batches": 200, "time_gap":   10, "num_neighbors": 20, "batch_size": 128},
    }
    
    if args.dataset_name in best_args_dict:
        if args.ablation == "num_fft_batches":
            args.time_gap, args.num_neighbors, args.batch_size = \
            best_args_dict[args.dataset_name]["time_gap"], \
            best_args_dict[args.dataset_name]["num_neighbors"], best_args_dict[args.dataset_name]["batch_size"]    

        else:
            args.num_fft_batches, args.time_gap, args.num_neighbors, args.batch_size = \
            best_args_dict[args.dataset_name]["num_fft_batches"], best_args_dict[args.dataset_name]["time_gap"], \
            best_args_dict[args.dataset_name]["num_neighbors"], best_args_dict[args.dataset_name]["batch_size"]
